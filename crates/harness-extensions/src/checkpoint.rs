...
#[derive(Default)]
pub struct MemoryCheckpoint { value: Mutex<Option<Value>>, execution: Arc<tokio::sync::Mutex<()>> }
impl CheckpointStore for MemoryCheckpoint {
    fn lease(&self) -> Result<Box<dyn StoreLease>> {
        Ok(Box::new(self.execution.clone().try_lock_owned().map_err(|_| Error::Conflict("checkpoint operation active".into()))?))
    }
    fn load(&self) -> Result<Option<Value>> { Ok(self.value.lock().map_err(|_| Error::Host("checkpoint lock".into()))?.clone()) }
    fn save(&self, value: &Value) -> Result<()> {
        *self.value.lock().map_err(|_| Error::Host("checkpoint lock".into()))? = Some(value.clone()); Ok(())
    }
}
...
pub struct FileCheckpoint { path: PathBuf, _lease: File, io_lock: Mutex<()>, execution: Arc<tokio::sync::Mutex<()>> }
impl FileCheckpoint {
    pub fn open(path: &Path) -> Result<Self> {
        let parent = path.parent().ok_or_else(|| Error::Invalid("checkpoint parent".into()))?;
        if !parent.is_dir() { return Err(Error::Invalid("provision private checkpoint directory first".into())); }
        ...
        FileExt::try_lock_exclusive(&lease).map_err(|_| Error::Conflict("graph is already leased".into()))?;
        Ok(Self { path: path.into(), _lease: lease, io_lock: Mutex::new(()), execution: Arc::new(tokio::sync::Mutex::new(())) })
    }
}
impl CheckpointStore for FileCheckpoint {
    ...
    fn load(&self) -> Result<Option<Value>> {
        let _lock = self.io_lock.lock().map_err(|_| Error::Host("checkpoint lock".into()))?;
        if !self.path.exists() { return Ok(None); }
        if std::fs::metadata(&self.path)?.len() > 16 * 1024 * 1024 { return Err(Error::Invalid("checkpoint too large".into())); }
    ...
    fn save(&self, value: &Value) -> Result<()> {
        ...
        if bytes.len() > 16 * 1024 * 1024 { return Err(Error::Invalid("checkpoint too large".into())); }
        ...
        let result = (|| -> Result<()> {
            ...
            #[cfg(unix)] { File::open(self.path.parent().ok_or_else(|| Error::Invalid("parent".into()))?)?.sync_all()?; }
            Ok(())
        })();
...
mod tests {
    use super::*;
    #[test] fn checkpoint_is_exclusive_and_survives_reopen() {
        let dir = tempfile::tempdir().unwrap(); let path = dir.path().join("cp.json");
        ...
        assert_eq!(FileCheckpoint::open(&path).unwrap().load().unwrap().unwrap()["revision"], 1);
    }
    #[test] fn corrupt_checkpoint_does_not_start_over() {
        let dir = tempfile::tempdir().unwrap(); let path = dir.path().join("cp.json");

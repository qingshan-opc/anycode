...
impl BudgetPool {
    pub fn new(limit: u64) -> Result<Self> {
        if limit == 0 { return Err(Error::Invalid("zero budget".into())); }
        Ok(Self(Arc::new(Mutex::new(BudgetSnapshot { limit, ..Default::default() }))))
    }
    ...
    pub fn from_checkpoint(mut state: BudgetSnapshot) -> Result<Self> {
        if state.limit == 0 { return Err(Error::Invalid("zero restored budget".into())); }
        state.spent = state.spent.checked_add(state.reserved).ok_or(Error::Budget)?;
        state.reserved = 0;
        state.overdrawn |= state.spent > state.limit;
        Ok(Self(Arc::new(Mutex::new(state))))
    }
    pub fn snapshot(&self) -> Result<BudgetSnapshot> {
        self.0.lock().map(|s| *s).map_err(|_| Error::Host("budget lock poisoned".into()))
    }
    pub fn reserve(&self, maximum_tokens: u64) -> Result<Reservation> {
        if maximum_tokens == 0 { return Err(Error::Invalid("zero reservation".into())); }
        let mut state = self.0.lock().map_err(|_| Error::Budget)?;
        let used = state.spent.checked_add(state.reserved).ok_or(Error::Budget)?;
        if state.overdrawn || maximum_tokens > state.limit.saturating_sub(used) {
            return Err(Error::Budget);
        }
        state.reserved += maximum_tokens;
        Ok(Reservation { pool: self.clone(), maximum_tokens, settled: false, sent: false })
    }
}
...
mod tests {
    use super::*;
    #[test] fn reservations_cannot_multiply_budget() {
    ...
    #[test] fn unknown_sent_usage_is_not_free() {
        let pool = BudgetPool::new(100).unwrap();
        { let mut r = pool.reserve(60).unwrap(); r.mark_sent(); }
        assert_eq!(pool.snapshot().unwrap().spent, 60);
        { let _r = pool.reserve(40).unwrap(); }
        assert_eq!(pool.snapshot().unwrap().reserved, 0);
    }
    #[test] fn overage_is_recorded_not_hidden() {
